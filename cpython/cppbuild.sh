...
case $PLATFORM in
    ...
    linux-arm64)
        ...
        make distclean
        export PATH=$INSTALL_PATH/host/bin/:$PATH
        CC="aarch64-linux-gnu-gcc -mabi=lp64" ./configure --prefix=$INSTALL_PATH --host=aarch64-linux-gnu --build=$(uname -m)-pc-linux-gnu --enable-shared --with-system-ffi --with-openssl=$INSTALL_PATH LDFLAGS='-s -Wl,-rpath,\$$ORIGIN/,-rpath,\$$ORIGIN/../,-rpath,\$$ORIGIN/../lib/' ac_cv_file__dev_ptmx=no ac_cv_file__dev_ptc=no --disable-ipv6
        make -j $MAKEJ
        make install
        ;;
    linux-ppc64le)
